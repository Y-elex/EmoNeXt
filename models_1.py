...
def get_model(num_classes, model_size="tiny", in_22k=False):
    ...
    out_dir = f"out/{model_size}"
    if os.path.exists(out_dir):
        model_files = glob.glob(os.path.join(out_dir, "*.pt"))
        latest_model = max(model_files, key=os.path.getctime)  # 按创建时间排序，获取最新模型
        print(f"加载已有模型: {latest_model}")
        net.load_state_dict(torch.load(latest_model, weights_only=True),strict=False)
    else:
        print("未找到已有模型，进行从头训练")
